...
import { getServiceConfig, E2E_FLOWS } from '../../../config/services-config.ts';
import { defaultMonitor } from '../../../utils/performance-monitor.ts';
import { testCreateAssessment, testSubmitAssessment, testGetResults } from '../services/health-assessment/health-assessment-api-test.ts';
import { testUploadSequence, testAnalyzeSequence, testGetResults as getGenomicsResults } from '../services/genomics/genomics-api-test.ts';
import { testCreateBooking, testGetBooking } from '../services/booking/booking-api-test.ts';
...
export default function(data) {
  ...
  try {
    ...
    if (sequenceId) {
      testAnalyzeSequence(sequenceId);
    }
    sleep(3);
    ...
    if (sequenceId) {
      getGenomicsResults(sequenceId);
    }
    sleep(1);
    
    if (bookingId) {
      testGetBooking(bookingId);
    }
